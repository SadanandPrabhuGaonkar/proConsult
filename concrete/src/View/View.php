        ...
        foreach ($outputAssets as $position => $assets) {
            ...
            foreach ($transformed as $item) {
                ...
                if (!in_array($itemstring, $outputItems)) {
                    $itemCustomString = $this->outputAssetIntoView($item);
                    if($item instanceof Asset ){
                        if ($item->getAssetHandle() != 'jquery') {
                            $itemstring = str_replace('<script ', '<script defer ', $itemstring);
                            $itemCustomString = str_replace('<script ', '<script defer ', $itemCustomString);
                        }
                    }

                    $output .= $itemCustomString;
