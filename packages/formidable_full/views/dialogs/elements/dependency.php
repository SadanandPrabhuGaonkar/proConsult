...
<?php } else { ?>
	...
	<?php if ($task == 'add') { ?>
		<fieldset class="dependency form-horizontal" id="dependency_rule_<?php echo $rule; ?>" data-rule="<?php echo $rule; ?>" data-elementID="<?php echo $current_element->getElementID(); ?>">
			<div class="input operator">
				<?php echo t('OR'); ?>
			</div>
			<div class="form-group">
				<label class="col-sm-3">
					<a href="javascript:ccmFormidableDeleteDependencyDialog(<?php echo $current_element->getElementID(); ?>, <?php echo $rule; ?>);" class="btn btn-danger option_button" title="<?php echo t('Delete this rule'); ?>">-</a>
			...
			<?php 
				if (!empty($dependency['actions'])) {
					foreach($dependency['actions'] as $action_rule => $action) {
						echo 'ccmFormidableAddDependencyAction('.$current_element->getElementID().', '.$rule.', '.$action_rule.');';		
					}
				}
			?> 
			<?php 
				if (!empty($dependency['elements'])) {
					foreach($dependency['elements'] as $element_rule => $element) {
						echo 'ccmFormidableAddDependencyElement('.$current_element->getElementID().', '.$rule.', '.$element_rule.');';								
					}
				} 
			?>
		</script>
	...
	<?php } elseif ($task == 'action') { ?>
		...
		<div class="dependency_action" id="action_<?php echo $action_rule; ?>" style="margin-top: 5px;">
			<div class="action_buttons" style="width:16%;float:right;">
				<a href="javascript:ccmFormidableAddDependencyAction(<?php echo $current_element->getElementID(); ?>, <?php echo $action_rule; ?>);" class="btn btn-success option_button" title="<?php echo t('Add an action to this rule'); ?>">+</a>
	...
	<?php } elseif ($task == 'element') { ?>
		...
		<div class="dependency_element" id="element_<?php echo $element_rule; ?>" style="margin-top: 5px;">
			...
			?>
			<div class="action_buttons" style="width:16%;float:right;">
				<a href="javascript:ccmFormidableAddDependencyElement(<?php echo $current_element->getElementID(); ?>, <?php echo $element_rule; ?>);" class="btn btn-success option_button" title="<?php echo t('Add an element to this rule'); ?>">+</a>
