Concrete.event.bind('btCoreValues.slides.edit.open', function (options, settings) {
    ...
    var ftImageLogo = $(newField).find('.ft-image-logo-file-selector');
            if ($(ftImageLogo).length > 0) {
                $(ftImageLogo).concreteFileSelector({'inputName': $(ftImageLogo).attr('data-file-selector-input-name'), 'filters': [], 'fID' : $(ftImageLogo).attr('data-file-selector-f-id') });
            }
            var pageSelectorLogo = $(newField).find('.ft-image-logo-page-selector');
            if ($(pageSelectorLogo).length > 0) {
                $(pageSelectorLogo).concretePageSelector({'inputName': $(pageSelectorLogo).attr('data-input-name'), 'cID' : $(pageSelectorLogo).attr('data-cID')});
            }
    };
