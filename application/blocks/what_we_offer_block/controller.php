<?php namespace Application\Block\WhatWeOfferBlock;
...
class Controller extends BlockController
{
    ...
    protected $btExportTables = ['btWhatWeOfferBlock', 'btWhatWeOfferBlockPointsEntries'];
    protected $btTable = 'btWhatWeOfferBlock';
    ...
    public function getSearchableContent()
    {
        ...
        $points_items = $db->fetchAll('SELECT * FROM btWhatWeOfferBlockPointsEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
        foreach ($points_items as $points_item_k => $points_item_v) {
            if (isset($points_item_v["txtpoint"]) && trim($points_item_v["txtpoint"]) != "") {
                $content[] = $points_item_v["txtpoint"];
            }
        }
    ...
    {
        $db = Database::connection();
        $db->delete('btWhatWeOfferBlockPointsEntries', ['bID' => $this->bID]);
        parent::delete();
    }
    ...
    public function duplicate($newBID)
    {
        ...
        foreach ($points_items as $points_item) {
            ...
            $db->insert('btWhatWeOfferBlockPointsEntries', $points_item);
        }
    ...
    protected function addEdit()
    {
        ...
        $al->register('css', 'repeatable-ft.form', 'blocks/what_we_offer_block/css_form/repeatable-ft.form.css', [], $this->pkg);
        $al->register('javascript', 'handlebars', 'blocks/what_we_offer_block/js_form/handlebars-v4.0.4.js', [], $this->pkg);
        $al->register('javascript', 'handlebars-helpers', 'blocks/what_we_offer_block/js_form/handlebars-helpers.js', [], $this->pkg);
    ...
    public function save($args)
    {
        ...
        $rows = $db->fetchAll('SELECT * FROM btWhatWeOfferBlockPointsEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
        ...
        if (!empty($points_items)) {
            ...
            foreach ($points_items as $points_item) {
                ...
                if (isset($points_item['txtpoint']) && trim($points_item['txtpoint']) != '') {
                    $data['txtpoint'] = trim($points_item['txtpoint']);
                } else {
                    $data['txtpoint'] = null;
                }
                if (isset($rows[$i])) {
                    ...
                    switch ($type) {
                        case 'update':
                            foreach ($values as $id => $data) {
                                $db->update('btWhatWeOfferBlockPointsEntries', $data, ['id' => $id]);
                            }
                            break;
                        case 'insert':
                            foreach ($values as $data) {
                                $db->insert('btWhatWeOfferBlockPointsEntries', $data);
                            }
                            break;
                        case 'delete':
                            foreach ($values as $value) {
                                $db->delete('btWhatWeOfferBlockPointsEntries', ['id' => $value]);
                            }
                            break;
                    }
                }
            ...
            if ($pointsEntriesErrors == 0) {
                foreach ($args['points'] as $points_k => $points_v) {
                    if (is_array($points_v)) {
                        if (in_array("txtpoint", $this->btFieldsRequired['points']) && (!isset($points_v['txtpoint']) || trim($points_v['txtpoint']) == "")) {
                            $e->add(t("The %s field is required (%s, row #%s).", t("Text point"), t("Points"), $points_k));
                        }
                    } else {
