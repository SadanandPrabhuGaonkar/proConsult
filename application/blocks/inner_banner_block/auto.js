Concrete.event.bind('inner_banner_block.stack.stacks', function () {
    $(document).ready(function () {
        $('select[name="stack[]"]').select2_sortable();
    });
});Concrete.event.bind('btInnerBannerBlock.bannerimgs.edit.open', function (options, settings) {
    ...
    var ftImageImgs = $(newField).find('.ft-image-imgs-file-selector');
            if ($(ftImageImgs).length > 0) {
                $(ftImageImgs).concreteFileSelector({'inputName': $(ftImageImgs).attr('data-file-selector-input-name'), 'filters': [], 'fID' : $(ftImageImgs).attr('data-file-selector-f-id') });
            }
            var pageSelectorImgs = $(newField).find('.ft-image-imgs-page-selector');
            if ($(pageSelectorImgs).length > 0) {
                $(pageSelectorImgs).concretePageSelector({'inputName': $(pageSelectorImgs).attr('data-input-name'), 'cID' : $(pageSelectorImgs).attr('data-cID')});
            }
    };
