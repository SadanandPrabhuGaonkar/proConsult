<?php namespace Application\Block\ContactPageBlock;
...
class Controller extends BlockController
{
    ...
    protected $btTable = 'btContactPageBlock';
    ...
    public function view()
    {
        ...
        $this->set('contentArea', LinkAbstractor::translateFrom($this->contentArea));
        $stack = [];
        if ($stack_entries = $db->fetchAll('SELECT * FROM btContactPageBlockStackEntries WHERE bID = ? ORDER BY sortOrder ASC', [$this->bID])) {
    ...
    {
        $db = Database::connection();
        $db->delete('btContactPageBlockStackEntries', ['bID' => $this->bID]);
        parent::delete();
    }
    ...
    {
        $db = Database::connection();
        $stack_entries = $db->fetchAll('SELECT * FROM btContactPageBlockStackEntries WHERE bID = ? ORDER BY sortOrder ASC', [$this->bID]);
        foreach ($stack_entries as $stack_entry) {
            unset($stack_entry['id']);
            $db->insert('btContactPageBlockStackEntries', $stack_entry);
        }
    ...
    public function edit()
    {
        ...
        $this->set('contentArea', LinkAbstractor::translateFromEditMode($this->contentArea));
        $stack_selected = [];
        $stack_ordered = [];
        $stack_options = $this->getStacks();
        if ($stack_entries = $db->fetchAll('SELECT * FROM btContactPageBlockStackEntries WHERE bID = ? ORDER BY sortOrder ASC', [$this->bID])) {
    ...
    protected function addEdit()
    {
        ...
        $al->register('javascript', 'select2sortable', 'blocks/contact_page_block/js_form/select2.sortable.js', [], $this->pkg);
    ...
    {
        $db = Database::connection();
        $args['contentArea'] = LinkAbstractor::translateTo($args['contentArea']);
        $stack_entries_db = [];
        $stack_queries = [];
        if ($stack_entries = $db->fetchAll('SELECT * FROM btContactPageBlockStackEntries WHERE bID = ? ORDER BY sortOrder ASC', [$this->bID])) {
            foreach ($stack_entries as $stack_entry) {
                    ...
                    switch ($type) {
                        case 'update':
                            foreach ($values as $id => $data) {
                                $db->update('btContactPageBlockStackEntries', $data, ['id' => $id]);
                            }
                            break;
                        case 'insert':
                            foreach ($values as $data) {
                                $db->insert('btContactPageBlockStackEntries', $data);
                            }
                            break;
                        case 'delete':
                            foreach ($values as $value) {
                                $db->delete('btContactPageBlockStackEntries', ['id' => $value]);
                            }
                            break;
                    }
                }
    ...
    public function validate($args)
    {
        ...
        if (in_array("contentArea", $this->btFieldsRequired) && (trim($args["contentArea"]) == "")) {
            $e->add(t("The %s field is required.", t("Content area")));
        }
